...
class YnrStack(Stack):
    ...
    def create_cloudfront(
        ...
        # Standard cache polity that allows CSRF headers and
        # doesn't do much else. Caps cache at 5 minutes, to ensure
        # we don't end up caching things for too long.
        no_cache_not_authenticated = cloudfront.CachePolicy(
            self,
            "no_cache_not_authenticated",
        ...
        cloudfront_dist.add_behavior(
            ...
            viewer_protocol_policy=cloudfront.ViewerProtocolPolicy.REDIRECT_TO_HTTPS,
            allowed_methods=cloudfront.AllowedMethods.ALLOW_ALL,
            cache_policy=no_cache_not_authenticated,
            compress=True,
        )
