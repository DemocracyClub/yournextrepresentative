...
class YNRPopItImporter(PopItImporter):
    ...
    def __init__(self, apps, schema_editor):
        ...
        self.election_cache = {e.slug: e for e in Election.objects.all()}
        self.uk_mapit_data = {}
    ...
    def get_images_for_object(self, images_data, django_extra_object):
        ...
        person_extra_content_type = ContentType.objects.get_for_model(
            django_extra_object
        )
        ...
        for image_data in images_data:
            with show_data_on_error("image_data", image_data):
                ...
                try:
                    ...
                    print(msg.format(url=url, status=e.response.status_code))
                    continue
                ...
                with open(image_filename, "rb") as f:
                    try:
                        ...
                        if "cannot identify image file" in e.args[0]:
                            print(
                                "Ignoring a non-image file {}".format(
                                    image_filename
                                )
                            )
                            continue
    ...
    def update_person(self, person_data):
        ...
        extra = PersonExtra.objects.create(
            base=person, versions=json.dumps(new_person_data["versions"])
        )
    ...
    def update_organization(self, org_data, area):
        org_id, org = super().update_organization(org_data, area)

        # Create the extra organization object:
        OrganizationExtra = self.get_model_class(
            "candidates", "OrganizationExtra"
        )
        register = org_data.get("register") or ""
        extra = OrganizationExtra.objects.create(
            base=org, slug=org_data["id"], register=register
        )
    ...
    def update_area(self, area_data):
        ...
        if (
            settings.ELECTION_APP == "uk_general_election_2015"
            or settings.ELECTION_APP == "ar_elections_2015"
            or settings.ELECTION_APP == "bf_elections_2015"
        ):
            identifier = new_area_data["identifier"]
            split_url = urlsplit(identifier)
            if not (
                split_url.netloc.endswith("mapit.mysociety.org")
                or split_url.netloc.endswith("mapit.staging.mysociety.org")
            ):
                raise Exception(
                    "Area identifers are expected to be MapIt area URLs"
                )
            ...
            if old_identifier == "/area/0":
                new_area_data[
                    "identifier"
                ] = "ocd-division/country:us/state:mn/place:st_paul"
            else:
                ...
                if m:
                    ...
                    message = (
                        "The format of the St Paul area ID was unexpected: {0}"
                    )
    ...
    def update_post(self, post_data, area, org_id_to_django_object):
        ...
        if len(area_types) > 1:
            message = (
                "Only one area type is allowed per election in this "
    ...
    def update_membership(
        ...
        if (not election_slug) and candidacy and membership.post:
            ...
            # Parlamentario Mercosur, there's a special case for that:
                ...
                raise Exception(
                    "Election missing on membership, and no unique matching election found"
                )
        if election_slug is not None:
            election = Election.objects.get(slug=election_slug)

            if membership.role == election.candidate_membership_role:
                MembershipExtra = self.get_model_class(
                    "candidates", "MembershipExtra"
                )
                me, created = MembershipExtra.objects.get_or_create(
                    base=membership, election=election
                )

                person_data = self.person_id_to_json_data[
                    new_membership_data["person_id"]
                ]
                party = person_data["party_memberships"].get(election.slug)
                if party is not None:
                    membership.on_behalf_of = org_id_to_django_object[
                        party["id"]
                    ]
                    membership.save()

                party_list_position = new_membership_data.get(
...
PARTY_SETS_BY_ELECTION_APP = {
    ...
    "bf_elections_2015": [{"slug": "national", "name": "National"}],
}
...
def import_from_popit(apps, schema_editor):
    ...
    reset_sql_list = connection.ops.sequence_reset_sql(no_style(), [Person])
    if reset_sql_list:
        ...
        with open(ar_filename) as f:
            ...
            for party_data in ar_all_party_data:
                ...
                if territory:
                    ...
                    ar_party_id_to_party_sets[party_data["id"]] = [party_set]
                else:
                    ar_party_id_to_party_sets[
                        party_data["id"]
    ...
    Membership.objects.filter(
        post__isnull=True, organization__classification="Party"
    ).delete()
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.RunPython(
            import_from_popit, lambda apps, schema_editor: None
        )
    ]
