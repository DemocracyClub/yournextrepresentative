...
class ExtraFieldTests(TestUserMixin, UK2015ExamplesMixin, WebTest):
    def setUp(self):
        ...
        p_field = ExtraField.objects.create(
            key="profession", label="Profession", type=ExtraField.LINE, order=3
        )
        ...
        self.person = Person.objects.create(
            name="John the Well-Described", versions="[]"
        )
        ...
        PersonExtraFieldValue.objects.create(
            field=p_field, person=self.person, value="Tree Surgeon"
        )
        PersonExtraFieldValue.objects.create(
            field=yn_field, person=self.person, value="yes"
        )
