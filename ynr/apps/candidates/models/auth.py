...
def check_creation_allowed(user, new_candidacies):
    for candidacy in new_candidacies:
        post = candidacy.post
        election = candidacy.extra.post_election.election
...
def check_update_allowed(user, old_name, old_candidacies, new_name, new_candidacies):
    ...
    # joining were locked:
    old_posts = set((
        c.post, c.extra.post_election.election) for c in old_candidacies)
    new_posts = set((
        c.post, c.extra.post_election.election) for c in new_candidacies)
