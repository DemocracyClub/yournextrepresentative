...
class TestMergePeople(TestCase):
    ...
    def test_merge_basic_unknown_details(self):
        ...
        self.assertEqual(
            merged, {"foo": "bar", "quux": "xyzzy", "hello": "goodbye"}
        )

    def test_merge_arrays(self):
        primary = {"some-list": ["a", "b", "c"]}
        secondary = {"some-list": ["b", "c", "d"]}
        merged = merge_popit_people(primary, secondary)
        self.assertEqual(merged, {"some-list": ["a", "b", "c", "d"]})

    def test_merge_array_primary_null(self):
        primary = {"some-list": None}
        secondary = {"some-list": ["a", "b", "c"]}
        merged = merge_popit_people(primary, secondary)
    ...
    def test_merge_conflicting_names(self):
        ...
        self.assertEqual(
            ...
            {
                "name": "Dave Cameron",
                "other_names": [{"name": "David Cameron"}],
            },
        )
    ...
    def test_merge_conflicting_names_previous_other_names(self):
        primary = {
            "name": "Dave Cameron",
            "other_names": [{"name": "David W D Cameron"}],
        }
        secondary = {
            "name": "David Cameron",
            "other_names": [{"name": "David William Donald Cameron"}],
        }
        merged = merge_popit_people(primary, secondary)
        self.assertEqual(set(merged.keys()), {"name", "other_names"})
        self.assertEqual(merged["name"], "Dave Cameron")
        sorted_other_names = sorted(
            merged["other_names"], key=lambda e: e["name"]
        )
        self.assertEqual(
            sorted_other_names,
            [
    ...
    def test_regression_merge_losing_memberships(self):
        ...
        primary = {
            ...
            "standing_in": {
                "2010": {"name": "Maidstone and The Weald", "post_id": "65936"},
                "2015": {
                    "elected": False,
                    "name": "Canterbury",
                    "post_id": "65878",
